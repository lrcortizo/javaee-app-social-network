...
import es.uvigo.esei.dgss.exercises.domain.UserFriendshipDTO;
...
public class UserEJB {
	...
	public List<User> searchUsers(String filter) {
		List<User> toRet = new ArrayList<>();
		toRet = em.createQuery("SELECT u FROM User u WHERE u.login LIKE :f OR u.name LIKE :f OR u.role LIKE :f",
				User.class).setParameter("f", "%" + filter + "%").getResultList();
		return toRet;
	}
	...
	public List<UserFriendshipDTO> getFrienshipRequests() {
		User user = findUserById(ctx.getCallerPrincipal().getName());

		List<UserFriendship> requests = em.createQuery(
				"SELECT uf FROM UserFriendship uf WHERE (user2 = :us OR user1 = :us) AND " + "accepted = FALSE)",
				UserFriendship.class).setParameter("us", user).getResultList();
		
		List<UserFriendshipDTO> toRet = new ArrayList<>();
		
		for(UserFriendship uf : requests) {
			toRet.add(uf.toDTO());
		}
	...
	public UserFriendship acceptFriendship(String login) {
		...
		UserFriendship toRet = em.find(UserFriendship.class, new UserFriendshipId(friend.getLogin(), user.getLogin()));
